...
public class Scene {
    ...
    final double GLOBAL_ILLUMINATION, ROUGHNESS_LIMIT;
    final int RECURSION_DEPTH;

    public Scene(double globalIllumination, int recursionDepth, double roughnessLimit, Camera camera, Sphere sky, Shape... shapes) {
        ...
        this.ROUGHNESS_LIMIT = roughnessLimit;
    ...
    public Color getColor(Intersection intersection, Ray ray, int recursionLimit) {
        ...
        // add roughness for optimization
        ray.accumulatedRoughness += intersection.shape.visualProperty.roughness;

        // roughness optimization
        if (ray.accumulatedRoughness < this.ROUGHNESS_LIMIT)
            ...
            if (intersection.shape.visualProperty.isReflective) {
                ...
                Ray reflectionRay = new Ray(reflectionOrigin, reflectionVector, ray.accumulatedRoughness);
